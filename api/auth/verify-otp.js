...
export default async function handler(req, res) {
    try {
        ...
        // Log para debugging
        console.log('[Proxy verify-otp] Raw req.body:', req.body);
        console.log('[Proxy verify-otp] req.body type:', typeof req.body);
        console.log('[Proxy verify-otp] Content-Type:', req.headers['content-type']);
        
        try {
            // Caso 1: Body es string (ya viene como form-urlencoded)
            if (typeof req.body === 'string') {
                body = req.body;
                console.log('[Proxy verify-otp] Body es string:', body);
            }
            // Caso 2: Body es Buffer
            else if (Buffer.isBuffer && Buffer.isBuffer(req.body)) {
                body = req.body.toString('utf-8');
                console.log('[Proxy verify-otp] Body es Buffer, convertido a string:', body);
            }
            // Caso 3: Body es objeto (parseado por Vercel)
                ...
                if (req.body.constructor === Object || Object.getPrototypeOf(req.body) === Object.prototype || Object.getPrototypeOf(req.body) === null) {
                    ...
                    console.log('[Proxy verify-otp] Body es objeto, convertido a form-urlencoded:', body);
                } else {
                    // Si no es un objeto simple, intentar convertir a string
                    body = String(req.body);
                    console.log('[Proxy verify-otp] Body es objeto complejo, convertido a string:', body);
                }
            }
            // Caso 4: Body es null o undefined
            else if (req.body === null || req.body === undefined) {
                console.warn('[Proxy verify-otp] Body es null o undefined');
                body = '';
            }
            // Caso 5: Otro tipo (number, boolean, etc.)
            else {
                body = String(req.body);
                console.log('[Proxy verify-otp] Body es otro tipo, convertido a string:', body);
            }
        } catch (bodyError) {
            console.error('[Proxy verify-otp] Error procesando body:', bodyError);
            console.error('[Proxy verify-otp] Error stack:', bodyError.stack);
            // Si hay error, intentar convertir a string como último recurso
            try {
                body = String(req.body || '');
            } catch (stringError) {
                console.error('[Proxy verify-otp] Error convirtiendo body a string:', stringError);
                body = '';
            }
        }
        ...
        if (!body || body.trim() === '') {
            console.error('[Proxy verify-otp] Body vacío o inválido');
            console.error('[Proxy verify-otp] req.body original:', req.body);
            console.error('[Proxy verify-otp] Content-Length:', req.headers['content-length']);
        ...
        console.log('[Proxy verify-otp] Enviando al backend:', {
            url: backendUrl,
            body: body,
        ...
        console.error('[Proxy verify-otp] Error stack:', error.stack);
        res.setHeader('Access-Control-Allow-Origin', '*');
        res.status(500).json({
